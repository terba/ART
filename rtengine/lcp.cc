                                     ...
                                     const LCPModelCommon &b, float facA)
{
    ...
    vign_param[2] =
        param0Sqr * param[0] - 2.0f * param[0] * param[1] + param[2];
    vign_param[3] = param0Sqr * param0Sqr + param[1] * param[1] +
...
{
    // Mention that the Adobe technical paper has a bug here, the DMAX is
    // handled differently for focLen and imgCenter
    ...
    if (foc_len_x < 0.0f) { // they may not be given
        ...
        foc_len_x = foc_len_y =
            focalLength /
            (35.0f * focalLength /
             focalLength35mm); // focLen must be calculated in pixels
    }
    ...
    // std::printf("FW %i /X0 %g   FH %i /Y0 %g
...
rtengine::LCPProfile::LCPProfile(const Glib::ustring &fname)
    : isFisheye(false), sensorFormatFactor(1.f), persModelCount(0),
      ...
      inInvalidTag{}, pCurPersModel(nullptr), pCurCommon(nullptr), aPersModel{}
{
    ...
    // Two phase filter: first filter out the very rough ones, that distord the
    // average a lot force it, even if there are few frames (community profiles)
    filterBadFrames(LCPCorrectionMode::VIGNETTE, 2.0, 0);
    filterBadFrames(LCPCorrectionMode::CA, 2.0, 0);
    // from the non-distorded, filter again on new average basis, but only if
    // there are enough frames left
                                      ...
                                      LCPModelCommon *pCorr3) const
{
    ...
    const float focalLengthLog = std::log(
        focalLength); //, apertureLog=aperture>0 ? std::log(aperture) : 0;
    const float focusDistLog = focusDist > 0 ? std::log(focusDist) + euler : 0;

    // Pass 1: determining best focal length, if possible different
    // focusDistances (for the focDist is not given case)
    for (int pm = 0; pm < persModelCount; ++pm) {
        const float f = aPersModel[pm]->focLen;

        if (aPersModel[pm]->hasModeData(mode)) {
            if (f <= focalLength &&
                (pLow == nullptr || f > pLow->focLen ||
                 (focusDist == 0 && f == pLow->focLen &&
                  pLow->focDist > aPersModel[pm]->focDist))) {
                pLow = aPersModel[pm];
            }

            if (f >= focalLength &&
                (pHigh == nullptr || f < pHigh->focLen ||
                 (focusDist == 0 && f == pHigh->focLen &&
    ...
    if (!pLow) {
        ...
        // Pass 2: We have some, so take the best aperture for vignette and best
        // focus for CA and distortion there are usually several frame per focal
        // length. In the end pLow will have both flen and apterure/focdis below
        // the target, and vice versa pHigh
        const float bestFocLenLow = pLow->focLen;
        const float bestFocLenHigh = pHigh->focLen;

        for (int pm = 0; pm < persModelCount; ++pm) {
            const float aper =
                aPersModel[pm]->aperture; // float aperLog=std::log(aper);
                ...
                if (aperture > 0 && mode != LCPCorrectionMode::CA) {
                    if (aPersModel[pm]->focLen == bestFocLenLow &&
                        ((aper == aperture && lowMeanErr > meanErr) ||
                         (aper >= aperture && aper < pLow->aperture &&
                          pLow->aperture > aperture) ||
                         (aper <= aperture &&
                          (pLow->aperture > aperture ||
                           fabs(aperture - aper) <
                               fabs(aperture - pLow->aperture))))) {
                        pLow = aPersModel[pm];
                    }

                    if (aPersModel[pm]->focLen == bestFocLenHigh &&
                        ((aper == aperture && highMeanErr > meanErr) ||
                         (aper <= aperture && aper > pHigh->aperture &&
                          pHigh->aperture < aperture) ||
                         (aper >= aperture &&
                          (pHigh->aperture < aperture ||
                           fabs(aperture - aper) <
                               fabs(aperture - pHigh->aperture))))) {
                        pHigh = aPersModel[pm];
                    }
                } else if (focusDist > 0 &&
                    ...
                    if (aPersModel[pm]->focLen == bestFocLenLow &&
                        ((focDist == focusDist && lowMeanErr > meanErr) ||
                         (focDist >= focusDist && focDist < pLow->focDist &&
                          pLow->focDist > focusDist) ||
                         (focDist <= focusDist &&
                          (pLow->focDist > focusDist ||
                           fabs(focusDistLog - focDistLog) <
                               fabs(focusDistLog -
                                    (std::log(pLow->focDist) + euler)))))) {
                        pLow = aPersModel[pm];
                    }

                    if (aPersModel[pm]->focLen == bestFocLenHigh &&
                        ((focDist == focusDist && highMeanErr > meanErr) ||
                         (focDist <= focusDist && focDist > pHigh->focDist &&
                          pHigh->focDist < focusDist) ||
                         (focDist >= focusDist &&
                          (pHigh->focDist < focusDist ||
                           fabs(focusDistLog - focDistLog) <
                               fabs(focusDistLog -
                                    (std::log(pHigh->focDist) + euler)))))) {
                        pHigh = aPersModel[pm];
                    }
                } else {
    ...
    if (pLow != nullptr && pHigh != nullptr) {
        ...
        float facLow = 0.5f;
        bool focLenOnSpot =
            false; // pretty often, since max/min are often as frames in LCP

        // There is as foclen range, take that as basis
        if (pLow->focLen < pHigh->focLen) {
            facLow = (std::log(pHigh->focLen) - focalLengthLog) /
                     (std::log(pHigh->focLen) - std::log(pLow->focLen));
        } else {
            focLenOnSpot =
                pLow->focLen == pHigh->focLen && pLow->focLen == focalLength;
        }

        // and average the other factor if available
        if (mode == LCPCorrectionMode::VIGNETTE && pLow->aperture < aperture &&
            pHigh->aperture > aperture) {
            // Mix in aperture
            const float facAperLow = (pHigh->aperture - aperture) /
                                     (pHigh->aperture - pLow->aperture);
            facLow =
                focLenOnSpot ? facAperLow : (0.5 * facLow + 0.5 * facAperLow);
        } else if (mode != LCPCorrectionMode::VIGNETTE && focusDist > 0 &&
                   pLow->focDist < focusDist && pHigh->focDist > focusDist) {
            // focus distance for all else (if focus distance is given)
            const float facDistLow =
                (std::log(pHigh->focDist) + euler - focusDistLog) /
                (std::log(pHigh->focDist) - std::log(pLow->focDist));
            facLow =
                focLenOnSpot ? facDistLow : (0.8 * facLow + 0.2 * facDistLow);
        }
        ...
        if (settings->verbose) {
            std::printf("LCP mode=%i, dist: %g found frames: Fno %g-%g; FocLen "
                        "%g-%g; Dist %g-%g with weight %g\n",
                        toUnderlying(mode), focusDist, pLow->aperture,
                        pHigh->aperture, pLow->focLen, pHigh->focLen,
                        pLow->focDist, pHigh->focDist, facLow);
        }
    } else {
        if (settings->verbose) {
            std::printf("Error: LCP file contained no %s parameters\n",
                        mode == LCPCorrectionMode::VIGNETTE     ? "vignette"
                        : mode == LCPCorrectionMode::DISTORTION ? "distortion"
                                                                : "CA");
        }
    }
}
    ...
    if (count >= minFramesLeft) {
        ...
        for (int pm = 0; pm < MaxPersModelCount && aPersModel[pm]; ++pm) {
            if (aPersModel[pm]->hasModeData(mode)) {
                switch (mode) {
                case LCPCorrectionMode::VIGNETTE: {
                    if (aPersModel[pm]->vignette.mean_error >
                        maxAvgDevFac * err) {
                ...
                case LCPCorrectionMode::CA: {
                    if (aPersModel[pm]->chromRG.mean_error >
                            maxAvgDevFac * err ||
                        aPersModel[pm]->chromG.mean_error >
                            maxAvgDevFac * err ||
                        aPersModel[pm]->chromBG.mean_error >
        ...
        if (settings->verbose && count) {
            std::printf(
                "Filtered %.1f%% frames for maxAvgDevFac %g leaving %i\n",
...
void rtengine::LCPProfile::handle_text(const std::string &text)
{
    ...
    } else if (tag == "RadialDistortParam4" ||
               tag == "TangentialDistortParam1") {
        pProf->pCurCommon->param[3] = std::atof(text.c_str());
    } else if (tag == "RadialDistortParam5" ||
               tag == "TangentialDistortParam2") {
                                                   ...
                                                   const char **attr)
{
    ...
    if (!pProf->inCamProfiles || pProf->inAlternateLensID ||
    ...
    if (src_str == "PerspectiveModel") {
        ...
        pProf->firstLIDone = true;
        pProf->inPerspect = true;
        pProf->isFisheye = true; // just misses third param, and different path,
                                 // rest is the same
        parseAttr = true;
    ...
    // some profiles (espc. Pentax) have a different structure that is
    // attributes based simulate tags by feeding them in
                                                  ...
                                                  const XML_Char *s, int len)
{
    ...
    if (!pProf->inCamProfiles || pProf->inAlternateLensID ||
        pProf->inAlternateLensNames || *pProf->inInvalidTag) {
        return;
    }
                                                 ...
                                                 const char *el)
{
    ...
    if (std::strstr(el, ":PerspectiveModel") ||
        std::strstr(el, ":FisheyeModel")) {
...
Glib::ustring rtengine::LCPStore::getDefaultCommonDirectory() const
{
    ...
    if (SHGetSpecialFolderPathW(NULL, pathW, CSIDL_COMMON_APPDATA, false)) {
        ...
        Glib::ustring fullDir =
            Glib::ustring(pathA) +
            Glib::ustring("\\Adobe\\CameraRaw\\LensProfiles\\1.0");
...
rtengine::LCPMapper::LCPMapper(const std::shared_ptr<LCPProfile> &pProf,
                               ...
                               int rawRotationDeg)
    : enableCA(false), useCADist(useCADistP), swapXY(false), isFisheye(false)
{
    if (!pProf) {
        return;
    }

    // determine in what the image with the RAW landscape in comparison
    // (calibration target) in vignetting, the rotation has not taken place yet
    int rot = 0;
    ...
    if (settings->verbose) {
        std::printf("Vign: %i, fullWidth: %i/%i, focLen %g SwapXY: %i / MirX/Y "
                    "%i / %i on rot:%i from %i\n",
                    vignette, fullWidth, fullHeight, focalLength, swapXY,
                    mirrorX, mirrorY, rot, rawRotationDeg);
    }

    pProf->calcParams(vignette ? LCPCorrectionMode::VIGNETTE
                               : LCPCorrectionMode::DISTORTION,
                      focalLength, focusDist, aperture, &mc, nullptr, nullptr);
    mc.prepareParams(fullWidth, fullHeight, focalLength, focalLength35mm,
                     pProf->sensorFormatFactor, swapXY, mirrorX, mirrorY);

    if (!vignette) {
        pProf->calcParams(LCPCorrectionMode::CA, focalLength, focusDist,
                          aperture, &chrom[0], &chrom[1], &chrom[2]);

        for (int i = 0; i < 3; ++i) {
            chrom[i].prepareParams(fullWidth, fullHeight, focalLength,
                                   focalLength35mm, pProf->sensorFormatFactor,
                                   swapXY, mirrorX, mirrorY);
        }
    }
                                            ...
                                            int cy, double scale) const
{
    ...
    if (isFisheye) {
        ...
        const double commonFac =
            (((aDist[2] * rsqr + aDist[1]) * rsqr + aDist[0]) * rsqr + 1.) +
            2. * (yfac * yd + xfac * xd);
                                    ...
                                    int channel) const
{
    ...
    if (channel == 1) {
        ...
        const double commonSum =
            1. + rsqr * (aCA[0] + rsqr * (aCA[1] + aCA[2] * rsqr)) +
            2. * (yfac * yd + xfac * xd);

        x = (chrom[channel].scale_factor * (xd * commonSum + xfac * rsqr)) *
                chrom[channel].fx +
            chrom[channel].x0;
        y = (chrom[channel].scale_factor * (yd * commonSum + yfac * rsqr)) *
                chrom[channel].fy +
            chrom[channel].y0;
    }
                                              ...
                                              float *line) const
{
    ...
    for (; x < width - 3; x += 4) {
        ...
        const vfloat vignFactorv =
            rsqr * (p0 + rsqr * (p1 - p2 * rsqr + p3 * rsqr * rsqr));
    ...
    for (; x < width; x++) {
        if (line[x] > 0) {
            ...
            line[x] +=
                line[x] * rsqr *
                (vignParam[0] + rsqr * ((vignParam[1]) - (vignParam[2]) * rsqr +
                                        (vignParam[3]) * rsqr * rsqr));
        }
    }
}
