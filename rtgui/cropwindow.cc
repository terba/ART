...
CropWindow::CropWindow(ImageArea *parent, bool isLowUpdatePriority_,
    ...
    : ObjectMOBuffer(parent), state(SNormal), press_x(0), press_y(0),
      action_x(0), action_y(0), pickedObject(-1), pickModifierKey(0),
      rot_deg(0), onResizeArea(false), deleted(false), fitZoomEnabled(true),
      fitZoom(false), cursor_type(CSArrow), prev_tool_mode(TMNone),
      /*isLowUpdatePriority(isLowUpdatePriority_),*/ hoveredPicker(nullptr),
      cropLabel(Glib::ustring("100%")), backColor(options.bgcolor),
      decorated(true), isFlawnOver(false), titleHeight(30), sideBorderWidth(3),
      lowerBorderWidth(3), upperBorderWidth(1), sepWidth(2), xpos(30), ypos(30),
      width(0), height(0), imgAreaX(0), imgAreaY(0), imgAreaW(0), imgAreaH(0),
      imgX(-1), imgY(-1), imgW(1), imgH(1), iarea(parent), cropZoom(0),
      zoomVersion(0), exposeVersion(0), cropgl(nullptr), pmlistener(nullptr),
      pmhlistener(nullptr), scrollAccum(0.0), observedCropWin(nullptr),
      crop_custom_ratio(0.f), area_updater_(nullptr)
{
    initZoomSteps();
    ...
    bZoomOut = new LWButton(
        Cairo::RefPtr<RTSurface>(new RTSurface("magnifier-minus-small.svg")), 0,
        nullptr, LWButton::Left, LWButton::Center, &zoomOuttt);
    bZoomIn = new LWButton(
        Cairo::RefPtr<RTSurface>(new RTSurface("magnifier-plus-small.svg")), 1,
        nullptr, LWButton::Left, LWButton::Center, &zoomIntt);
    bZoom100 = new LWButton(
        Cairo::RefPtr<RTSurface>(new RTSurface("magnifier-1to1-small.svg")), 2,
        nullptr, LWButton::Left, LWButton::Center, &zoom100tt);
    // bZoomFit = new LWButton (Cairo::RefPtr<RTSurface>(new
    // RTSurface("magnifier-fit.svg")), 3, NULL, LWButton::Left,
    // LWButton::Center, "Zoom Fit");
    bClose = new LWButton(
        Cairo::RefPtr<RTSurface>(new RTSurface("cancel-small.svg")), 4, nullptr,
        LWButton::Right, LWButton::Center, &closett);
...
void CropWindow::initZoomSteps()
{
    ...
    char lbl[64];
    // NOTE(zoulu): 1000 is the watershed for zoomed in and zoomed out, >=1000
    // (inclusive) indicates zoomed in.
...
void CropWindow::setPosition(int x, int y)
{
    ...
    if (decorated) {
        buttonSet.arrangeButtons(xpos + sideBorderWidth,
                                 ...
                                 width - 2 * sideBorderWidth, titleHeight);
    }
}
...
void CropWindow::setSize(int w, int h, bool norefresh)
{
    ...
    if (decorated) {
        ...
        imgAreaH = height - lowerBorderWidth - titleHeight - sepWidth -
                   upperBorderWidth;
        buttonSet.arrangeButtons(xpos + sideBorderWidth,
                                 ypos + upperBorderWidth,
                                 width - 2 * sideBorderWidth, titleHeight);
...
void CropWindow::leaveNotify(GdkEventCrossing *event)
{
    ...
    if (state == SNormal && subscriber &&
        subscriber->getEditingType() == ET_PIPETTE) {
                        ...
                        double deltaX, double deltaY)
{
    ...
    if (direction == GDK_SCROLL_SMOOTH) {
        scrollAccum += delta;
        // Only change zoom level if we've accumulated +/- 1.0 of deltas.  This
        // conditional handles the previous delta=0.0 case
    ...
    if (iarea->getToolMode() == TMHand && editSubscriber &&
        editSubscriber->getEditingType() == ET_OBJECTS && cropgl) {
        bool propagate = true;
        if (editSubscriber->scroll(state, direction, deltaX, deltaY,
                                   propagate)) {
            iarea->redraw();
        }
        if (!propagate) {
    ...
    bool isUp = direction == GDK_SCROLL_UP ||
                (direction == GDK_SCROLL_SMOOTH && scrollAccum < 0.0);
    scrollAccum = 0.0;
                             ...
                             double pressure)
{
    ...
    if (button == 1) {
        if (type == GDK_2BUTTON_PRESS && onArea(CropImage, x, y) &&
            iarea->getToolMode() != TMColorPicker &&
            (state == SNormal || state == SCropImgMove)) {
            if (fitZoomEnabled) {
                if (fitZoom) {
                    state = SNormal;
            ...
            if (onArea(CropToolBar, x, y)) {
                ...
                if (onArea(CropImage, x,
                           y)) { // events inside of the image domain
                    crop_custom_ratio = 0.f;
                    if ((bstate & GDK_SHIFT_MASK) &&
                        cropHandler.cropParams.w > 0 &&
                        cropHandler.cropParams.h > 0) {
                        crop_custom_ratio = float(cropHandler.cropParams.w) /
                                            float(cropHandler.cropParams.h);
                    }

                    const bool crop_mode = iarea->getToolMode() == TMCropSelect;
                    const bool crop_resize_allowed =
                        (crop_mode ? true : (bstate & GDK_CONTROL_MASK)) &&
                        !(bstate & GDK_SHIFT_MASK);
                    const bool crop_move_allowed =
                        crop_mode ? (!(bstate & GDK_CONTROL_MASK) &&
                                     !(bstate & GDK_SHIFT_MASK))
                                  : (bstate & GDK_SHIFT_MASK);
                    ...
                    if (iarea->getToolMode() == TMHand && area_updater_) {
                        ...
                        area_updater_->updateRectangleArea(
                            AreaDrawUpdater::BEGIN, press_x, press_y,
                            press_x + 1, press_y + 1);
                        ...
                        if (hoveredPicker) {
                            if ((bstate & GDK_CONTROL_MASK) &&
                                ...
                                updateHoveredPicker();
                                needRedraw = true;
                            } else if (!(bstate & GDK_CONTROL_MASK) &&
                                       (bstate & GDK_SHIFT_MASK)) {
                            ...
                            LockableColorPicker *newPicker =
                                new LockableColorPicker(
                                    this,
                                    &cropHandler.colorParams.outputProfile,
                                    &cropHandler.colorParams.workingProfile);
                            ...
                            needRedraw = true;
                               ...
                               editSubscriber) {
                        if ((cropgl &&
                             cropgl->inImageArea(iarea->posImage.x,
                                                 iarea->posImage.y) &&
                             (editSubscriber->getEditingType() == ET_PIPETTE &&
                              (bstate & GDK_CONTROL_MASK))) ||
                            editSubscriber->getEditingType() == ET_OBJECTS) {
                            needRedraw = editSubscriber->button1Pressed(
                                bstate, pressure);
                    ...
                    } else if (crop_resize_allowed &&
                               onArea(CropTopLeft, x, y)) {
                        state = SResizeTL;
                        press_x = x;
                    ...
                    } else { // if(zoomSteps[cropZoom].zoom >
                             // cropHandler.getFitZoom()) { // only allow move
                             // when image is only partial visible
                        state = SCropImgMove;
                        press_x = x;
                        press_y = y;
                        action_x = 0;
                ...
                } else if (iarea->getToolMode() ==
                           TMHand) { // events outside of the image domain
                    EditSubscriber *editSubscriber = iarea->getCurrSubscriber();

                    if (editSubscriber &&
                        editSubscriber->getEditingType() == ET_OBJECTS) {
                        needRedraw =
                            editSubscriber->button1Pressed(bstate, pressure);
        ...
        if (iarea->getToolMode() == TMHand) {
            ...
            if (editSubscriber &&
                ...
                action_y = 0;
                   ...
                   type == GDK_BUTTON_PRESS && state == SNormal) {
            if (hoveredPicker) {
                if ((bstate & GDK_CONTROL_MASK) && (bstate & GDK_SHIFT_MASK)) {
                    ...
                    for (std::vector<LockableColorPicker *>::iterator i =
                             colorPickers.begin();
                         i != colorPickers.end(); ++i) {
                        if (*i == hoveredPicker) {
...
void CropWindow::buttonRelease(int button, int num, int bstate, int x, int y)
{
    ...
    if (state == SCropWinResize) {
        ...
        for (std::list<CropWindowListener *>::iterator i = listeners.begin();
             i != listeners.end(); ++i) {
        ...
        state = SNormal;
        needRedraw = true;
    } else if (state == SEditDrag1 || state == SEditDrag2 ||
               state == SEditDrag3) {
        if (editSubscriber) {
            if (state == SEditDrag1) {
            ...
            if (state == SEditDrag1 &&
                ...
                iarea->object =
                    onArea(CropImage, x, y) && !onArea(CropObserved, x, y) ? 1
                                                                           : 0;

                // iarea->object = cropgl &&
                // cropgl->inImageArea(iarea->posImage.x, iarea->posImage.y) ? 1
                // : 0;
                if (iarea->object) {
                    crop->getPipetteData(iarea->pipetteVal, cropPos.x,
                                         cropPos.y,
                                         iarea->getPipetteRectSize());
                    // printf("PipetteData:  %.3f  %.3f  %.3f\n",
                    // iarea->pipetteVal[0], iarea->pipetteVal[1],
                    // iarea->pipetteVal[2]);
                } else {
                    iarea->pipetteVal[0] = iarea->pipetteVal[1] =
                        iarea->pipetteVal[2] = -1.f;
                }
    ...
    } else if (state == SEditPick1 || state == SEditPick2 ||
               state == SEditPick3) {
        if (editSubscriber) {
            Coord imgPos;
            action_x = x;
            ...
            bool elemPicked = iarea->object == pickedObject &&
                              (bstate & 255) == pickModifierKey;
        ...
        needRedraw = true;
    } else if (state == SNormal && iarea->getToolMode() == TMColorPicker &&
               !hoveredPicker && button == 3) {
        iarea->setToolHand();
    } else if (button == 1 && state == SCropSelecting && area_updater_) {
        screenCoordToImage(x, y, action_x, action_y);
        iarea->setToolHand();
        area_updater_->updateRectangleArea(AreaDrawUpdater::END, press_x,
                                           press_y, action_x, action_y);
        area_updater_ = nullptr;
        needRedraw = true;
        state = SNormal;
    }
    ...
    if (cropgl &&
        (state == SCropSelecting || state == SResizeH1 || state == SResizeH2 ||
         state == SResizeW1 || state == SResizeW2 || state == SResizeTL ||
         state == SResizeTR || state == SResizeBL || state == SResizeBR ||
         state == SCropMove)) {
    ...
    if (state != SDeletePicker && state != SEditDrag3 && state != SEditPick3 &&
        button == 3 && !(bstate & (GDK_SHIFT_MASK | GDK_CONTROL_MASK))) {
        iarea->pipetteVal[0] = iarea->pipetteVal[1] = iarea->pipetteVal[2] =
            -1.f;
...
void CropWindow::pointerMoved(int bstate, int x, int y, double pressure)
{
    ...
    if (state == SCropWinMove) {
        ...
        // multiplier is the amplification factor ; disabled if the user
        // selected "1" (no amplification)
        double factor = options.panAccelFactor == 1
                            ? 1.0
                            : options.panAccelFactor * zoomSteps[cropZoom].zoom;
        ...
        cropgl->cropHeight1Resized(cropHandler.cropParams.x,
                                   ...
                                   cropHandler.cropParams.h, crop_custom_ratio);
        iarea->redraw();
        ...
        if (area_updater_) {
            area_updater_->updateRectangleArea(AreaDrawUpdater::UPDATE, cx1,
                                               cy1, cx2, cy2);
        } else {
        ...
        if (state == SNormal || state == SEditPick1 || state == SEditPick2 ||
            ...
            // oldPosImage.x, oldPosImage.y, iarea->posImage.x,
            // iarea->posImage.y, iarea->deltaImage.x, iarea->deltaImage.y);
            ...
            // printf("          action_ & xy (%d x %d) -> (%d x %d) = (%d x %d)
            // + (%d x %d) / deltaPrev(%d x %d)\n", action_x, action_y,
            // currPos.x, currPos.y, iarea->posImage.x, iarea->posImage.y,
            // iarea->deltaImage.x, iarea->deltaImage.y,
            // iarea->deltaPrevImage.x, iarea->deltaPrevImage.y);

            Coord oldPosScreen = iarea->posScreen + iarea->deltaScreen;
            // printf(">>> SCR / ScrPrev(%d x %d) = (%d x %d) + (%d x %d)\n",
            // oldPosScreen.x, oldPosScreen.y, iarea->posScreen.x,
            // iarea->posScreen.y, iarea->deltaScreen.x, iarea->deltaScreen.y);
            currPos.set(x, y);
            iarea->deltaScreen = currPos - iarea->posScreen;
            iarea->deltaPrevScreen = currPos - oldPosScreen;
            // printf("          action_ & xy (%d x %d) -> (%d x %d) = (%d x %d)
            // + (%d x %d) / deltaPrev(%d x %d)\n", action_x, action_y,
            // currPos.x, currPos.y, iarea->posScreen.x, iarea->posScreen.y,
            // iarea->deltaScreen.x, iarea->deltaScreen.y,
            // iarea->deltaPrevScreen.x, iarea->deltaPrevScreen.y);
    ...
    if (pmlistener) {
        ...
        if (!onArea(CropImage, x, y) || !cropHandler.cropPixbuftrue) {
            ...
            pmlistener->pointerMoved(
                false, cropHandler.colorParams.outputProfile,
                cropHandler.colorParams.workingProfile, mx, my, -1, -1, -1);

            if (pmhlistener) {
                pmhlistener->pointerMoved(
                    false, cropHandler.colorParams.outputProfile,
                    cropHandler.colorParams.workingProfile, mx, my, -1, -1, -1);
            }
            ...
            if (vx < imwidth && vy < imheight) {
                guint8 *pix = cropHandler.cropPixbuftrue->get_pixels() +
                              vy * cropHandler.cropPixbuftrue->get_rowstride() +
                              vx * 3;
                int rval = pix[0];
                int gval = pix[1];
                int bval = pix[2];
                ...
                if (ipc) {
                    ...
                    isRaw = params.raw.bayersensor.method ==
                                RAWParams::BayerSensor::Method::NONE ||
                            params.raw.xtranssensor.method ==
                                RAWParams::XTransSensor::Method::NONE;
                    if (isRaw) {
                        ImageSource *isrc =
                            static_cast<ImageSource *>(ipc->getInitialImage());
                        isrc->getRawValues(mx, my, params.coarse.rotate, rval,
                                           gval, bval);
                    }
                }

                // Updates the Navigator
                // TODO: possible double color conversion if rval, gval, bval
                // come from cropHandler.cropPixbuftrue ? see issue #4583
                pmlistener->pointerMoved(true,
                                         cropHandler.colorParams.outputProfile,
                                         cropHandler.colorParams.workingProfile,
                                         mx, my, rval, gval, bval, isRaw);

                if (pmhlistener) {
                    // Updates the HistogramRGBArea
                    pmhlistener->pointerMoved(
                        true, cropHandler.colorParams.outputProfile,
                        cropHandler.colorParams.workingProfile, mx, my, rval,
                        gval, bval);
                }
            }
        }
...
bool CropWindow::onArea(CursorArea a, int x, int y)
{
    ...
    case CropBorder:
        return (x >= xpos + imgAreaX && y >= ypos + imgAreaY &&
                x < xpos + imgAreaX + imgAreaW &&
                y < ypos + imgAreaY + imgAreaH) &&
               !(x >= xpos + imgX && y >= ypos + imgY &&
                 x < xpos + imgX + imgW && y < ypos + imgY + imgH);
    ...
    case CropResize:
        return decorated && x >= xpos + width - 16 && y >= ypos + height - 16 &&
...
void CropWindow::updateCursor(int x, int y, int bstate)
{
    ...
    const bool crop_resize_allowed =
        (crop_mode ? true : (bstate & GDK_CONTROL_MASK)) &&
        !(bstate & GDK_SHIFT_MASK) &&
        !(editSubscriber && editSubscriber->getEditingType() == ET_OBJECTS);
    const bool crop_move_allowed =
        crop_mode ? (!(bstate & GDK_CONTROL_MASK) && !(bstate & GDK_SHIFT_MASK))
                  : (bstate & GDK_SHIFT_MASK);
    ...
    if (state == SNormal) {
        if (onArea(CropWinButtons, x, y)) {
            ...
            newType = CSMove;
        } else if (/*iarea->getObject() > -1 &&*/ editSubscriber &&
                   editSubscriber->getEditingType() == ET_OBJECTS) {
            int cursorX;
            int cursorY;
            screenCoordToImage(x, y, cursorX, cursorY);
            newType =
                editSubscriber->getCursor(iarea->getObject(), cursorX, cursorY);
        } else if (onArea(CropResize, x, y)) {
            ...
            newType = CSResizeBottomRight;
                   ...
                   (onArea(CropTop, x, y) || onArea(CropBottom, x, y))) {
            newType = CSResizeHeight;
        } else if (crop_resize_allowed &&
                   (onArea(CropLeft, x, y) || onArea(CropRight, x, y))) {
            ...
            if (editSubscriber &&
                editSubscriber->getEditingType() == ET_OBJECTS) {
                Coord cropPos;
                screenCoordToCropCanvas(iarea->posScreen.x, iarea->posScreen.y,
                                        cropPos.x, cropPos.y);
        ...
        newType = CSStraighten;
    } else if (state == SCropMove || state == SCropWinMove ||
               state == SObservedMove) {
        newType = CSMove;
    ...
    const auto is_resize = [](CursorShape s) -> bool {
        switch (s) {
    ...
    enum { CHANGE_CROP_NONE, CHANGE_CROP_GUIDES, CHANGE_CROP_FULL };
    int change_crop_state = CHANGE_CROP_NONE;
    if (tm != prev_tool_mode &&
        (tm == TMCropSelect || prev_tool_mode == TMCropSelect)) {
...
void show_focus_mask(Glib::RefPtr<Gdk::Pixbuf> pixbuf,
                     Glib::RefPtr<Gdk::Pixbuf> pixbuftrue)
{
    addFocusMask(pixbuftrue->get_pixels(), pixbuf->get_pixels(),
                 pixbuf->get_width(), pixbuf->get_height(),
                 pixbuftrue->get_rowstride(), pixbuf->get_rowstride(), 0, 0);
}

void get_rgb(const char *color, guint8 *rgb)
{
    const auto get = [](const char *color, int i) -> guint8 {
                       ...
                       Glib::RefPtr<Gdk::Pixbuf> pixbuftrue)
{
    ...
    const auto get_L = [](guint8 *rgb) -> int {
...
void CropWindow::expose(Cairo::RefPtr<Cairo::Context> cr)
{
    ...
    if (backColor == 0) {
        style->render_background(cr, x + imgAreaX, y + imgAreaY, imgAreaW,
                                 imgAreaH);
    } else {
        if (backColor == 1) {
    ...
    bool tweak_crop_guides =
        (/*state == SNormal &&*/ iarea->getToolMode() != TMCropSelect);
    ...
    const bool editing_crop = (tweak_crop_guides || state == SDragPicker ||
                               state == SDeletePicker || state == SEditDrag1);
    if (tweak_crop_guides) {
    ...
    if (state == SCropImgMove || state == SCropWinResize) {
        ...
        if (cropHandler.cropPixbuf) {
            ...
            const bool showR =
                iarea->previewModePanel
                    ->showR(); // will show clipping if R channel is clipped
            const bool showG =
                iarea->previewModePanel
                    ->showG(); // will show clipping if G channel is clipped
            const bool showB =
                iarea->previewModePanel
                    ->showB(); // will show clipping if B channel is clipped
            const bool showL =
                iarea->previewModePanel
                    ->showL(); // will show clipping if L value   is clipped
            const bool showFocusMask = iarea->indClippedPanel->showFocusMask();
            const bool showFalseColors =
                iarea->indClippedPanel->showFalseColors() &&
                !(showL || showR || showG || showB);
            bool showcs = iarea->indClippedPanel->showClippedShadows();
            bool showch = iarea->indClippedPanel->showClippedHighlights();

            // While the Right-side ALT is pressed, auto-enable highlight and
            // shadow clipping indicators
            ...
            if (!mask_shown && (showcs || showch || showR || showG || showB ||
                ...
                const int pixWSRowStride =
                    cropHandler.cropPixbuftrue->get_rowstride();
                ...
                if (showFocusMask) { // modulate preview to display focus mask
                    ...
                    const float ShawdowFac =
                        64.f / (options.shadowThreshold + 1);
                    const float HighlightFac =
                        64.f / (256 - options.highlightThreshold);
                    const bool showclippedAny =
                        (!showR && !showG && !showB &&
                         !showL); // will show clipping if any (all) of RGB
                                  // channels is (shadow) clipped
...
#pragma omp parallel for schedule(dynamic, 16)
                    ...
                    for (int i = 0; i < bHeight; i++) {
                        ...
                        for (int j = 0; j < bWidth; j++) {
                            // we must compare clippings in working space, since
                            // the cropPixbuf is in sRGB, with mon profile

                            bool changedHL = false;
                            bool changedSH = false;
                            int delta = 0;
                            // for efficiency, pre-calculate currWS_L as it may
                            // be needed in both if (showch) and if (showcs)
                            // branches
                            int currWS_L = 0;

                            if (showL && (showch || showcs)) {
                                currWS_L = (int)(0.299f * currWS[0] +
                                                 0.587f * currWS[1] +
                                                 0.114f * currWS[2]);
                            }

                            if (showch) {
                                if ((showclippedAny || showR) &&
                                    currWS[0] >= hlThreshold) {
                                    delta += 255 - currWS[0];
                                    changedHL = true;
                                }

                                if ((showclippedAny || showG) &&
                                    currWS[1] >= hlThreshold) {
                                    delta += 255 - currWS[1];
                                    changedHL = true;
                                }

                                if ((showclippedAny || showB) &&
                                    currWS[2] >= hlThreshold) {
                                    delta += 255 - currWS[2];
                                    changedHL = true;
                                }
                                    ...
                                    if (showclippedAny) {
                                        if (has_ch_color) {
                                            ...
                                            curr[0] = curr[1] = curr[2] =
                                                delta; // indicate clipped
                                                       // highlights in gray
                                        }
                                    } else {
                                        curr[0] = 255; // indicate clipped
                                                       // highlights in red
                                        curr[1] = curr[2] = delta;
                                    }
                                }
                            ...
                            if (showcs) {
                                ...
                                if (changedSH) {
                                    if (showclippedAny) {
                                        ...
                                        curr[2] = 255;
                                        curr[0] = curr[1] =
                                            delta; // indicate clipped shadows
                                                   // in blue
                                    }
                                }
                            } // if (showcs)
                            ...
                            if (!changedHL && !changedSH &&
                                !showclippedAny) { // This condition allows
                                                   // clipping indicators for
                                                   // RGB channels to remain in
                                                   // color
                                if (showR) {
                                    curr[1] = curr[2] =
                                        curr[0]; // Red   channel in grayscale
                                }

                                if (showG) {
                                    curr[0] = curr[2] =
                                        curr[1]; // Green channel in grayscale
                                }

                                if (showB) {
                                    curr[0] = curr[1] =
                                        curr[2]; // Blue  channel in grayscale
                                }
                                ...
                                if (showL) { // Luminosity
                                    ...
                                    // for more info
                                    // int L =
                                    // (int)(0.212671*curr[0]+0.715160*curr[1]+0.072169*curr[2]);
                                    int L =
                                        (int)(0.299 * curr[0] +
                                              0.587 * curr[1] +
                                              0.114 *
                                                  curr[2]); // Lightness - this
                                                            // matches
                                                            // Luminosity mode
                                                            // in Photoshop CS5
            ...
            if (editSubscriber &&
                ...
                if (this != iarea->mainCropWindow) {
                    cr->set_line_width(0.);
                    cr->rectangle(x + imgAreaX, y + imgAreaY, imgAreaW,
                                  imgAreaH);
                    cr->clip();
                }

                // drawing Subscriber's visible geometry
                const std::vector<Geometry *> visibleGeom =
                    editSubscriber->getVisibleGeometry();
                cr->set_antialias(
                    Cairo::ANTIALIAS_DEFAULT); // ANTIALIAS_SUBPIXEL ?
                ...
                const auto mouseOverGeom =
                    editSubscriber->getMouseOverGeometry();
                if (mouseOverGeom.size()) {
                    if (mouseOverGeom.size() > 65534) {
                        // once it has been switched to OM_65535, it won't
                        // return back to OM_255 to avoid constant memory
                        // allocations in some particular situation. It will
                        // return to OM_255 on a new editing session
                        setObjectMode(OM_65535);
                    }

                    Cairo::RefPtr<Cairo::Context> crMO =
                        Cairo::Context::create(ObjectMOBuffer::getObjectMap());
                    ...
                    crMO->rectangle(
                        0., 0., ObjectMOBuffer::getObjectMap()->get_width(),
                        ObjectMOBuffer::getObjectMap()->get_height());
    ...
    if (state == SNormal && isFlawnOver) {
        ...
        if (iarea->getToolMode() == TMHand && editSubscriber &&
            editSubscriber->getEditingType() == ET_PIPETTE && iarea->object) {
    ...
    if ((state == SNormal || state == SDragPicker) && isPreviewImg &&
        iarea->showColorPickers()) {
        for (auto colorPicker : colorPickers) {
            colorPicker->draw(cr);
        }
    }
...
void CropWindow::drawGridOverlay(Cairo::RefPtr<Cairo::Context> cr)
{
    if (this == iarea->mainCropWindow) {
        ...
        cparams.h = imgAreaH;
        Glib::RefPtr<Gtk::StyleContext> style = iarea->get_style_context();
        drawCrop(style, cr, xpos + imgAreaX, ypos + imgAreaY, imgAreaW,
                 imgAreaH, 0, 0, 1, cparams, true, true, false);
    }
}
    ...
    if (toCursor) {
        ...
        if (zoomSteps[cropZoom].zoom <= cropHandler.getFitZoom()) {
            ...
            if (cropHandler.cropParams.enabled) {
                ...
                int x1 =
                    cropHandler.cropParams.x + cropHandler.cropParams.w / 2;
                int y1 =
...
double CropWindow::getZoom() { return zoomSteps[cropZoom].zoom; }

bool CropWindow::isMinZoom() { return cropZoom <= 0; }

bool CropWindow::isMaxZoom() { return cropZoom >= int(zoomSteps.size()) - 1; }
...
void CropWindow::updateHoveredPicker(rtengine::Coord *imgPos)
{
    ...
    LockableColorPicker::Validity validity =
        checkValidity(hoveredPicker, cropPos);
...

    {
        ...
        if (validity == LockableColorPicker::Validity::INSIDE) {
            cropHandler.colorPick(cropPos, r, g, b, rpreview, gpreview,
                                  bpreview, hoveredPicker->getSize());
                                        ...
                                        int &phyy)
{
    ...
    phyy = (imgy - cropY) * zoomSteps[cropZoom].zoom +
           /*ypos + imgY +*/ crop->getUpperBorder();
}

void CropWindow::imageCoordToCropBuffer(double imgx, double imgy, double &phyx,
                                        double &phyy)
{
    int cropX, cropY;
    rtengine::Crop *crop = static_cast<rtengine::Crop *>(cropHandler.getCrop());
    ...
    phyy = (imgy - double(cropY)) * zoomSteps[cropZoom].zoom +
           double(/*ypos + imgY +*/ crop->getUpperBorder());
}
...
void CropWindow::imageCoordToCropImage(double imgx, double imgy, double &phyx,
                                       double &phyy)
{
    int cropX, cropY;
    cropHandler.getPosition(cropX, cropY);
...
void CropWindow::drawDecoration(Cairo::RefPtr<Cairo::Context> cr)
{
    ...
    fontd.set_size(8 * Pango::SCALE);
    context->set_font_description(fontd);
    Glib::RefPtr<Pango::Layout> cllayout =
        iarea->create_pango_layout(cropLabel);
    int iw, ih;
    cllayout->get_pixel_size(iw, ih);
    ...
    cr->move_to(x + 10 + sideBorderWidth + bZoomIn->getIcon()->getWidth() +
                    bZoomOut->getIcon()->getWidth() +
                    bZoom100->getIcon()->getWidth(),
                y + 1 + upperBorderWidth + (titleHeight - ih) / 2);
...
void CropWindow::drawStraightenGuide(Cairo::RefPtr<Cairo::Context> cr)
{
    ...
    Glib::RefPtr<Pango::Layout> deglayout =
        iarea->create_pango_layout(Glib::ustring::compose(
            "%1 deg", Glib::ustring::format(std::setprecision(2), rot_deg)));
...
void CropWindow::drawScaledSpotRectangle(Cairo::RefPtr<Cairo::Context> cr,
                                         int rectSize)
{
    ...
    cr->set_line_width(1.0);
    cr->rectangle(xpos + imgX + imgAreaX - 0.5, ypos + imgY + imgAreaY - 0.5,
                  imgAreaW, imgAreaH);
    cr->clip();

    cr->set_source_rgb(1.0, 1.0, 1.0);
    cr->rectangle(
        x1 * zoomSteps[cropZoom].zoom - 1.5,
        y1 * zoomSteps[cropZoom].zoom - 1.5,
        x2 * zoomSteps[cropZoom].zoom - x1 * zoomSteps[cropZoom].zoom + 2,
        y2 * zoomSteps[cropZoom].zoom - y1 * zoomSteps[cropZoom].zoom + 2);
    cr->stroke();
    cr->set_source_rgb(0.0, 0.0, 0.0);
    cr->rectangle(x1 * zoomSteps[cropZoom].zoom - 0.5,
                  y1 * zoomSteps[cropZoom].zoom - 0.5,
                  x2 * zoomSteps[cropZoom].zoom - x1 * zoomSteps[cropZoom].zoom,
                  y2 * zoomSteps[cropZoom].zoom -
                      y1 * zoomSteps[cropZoom].zoom);
    cr->stroke();

    cr->reset_clip();
}

void CropWindow::drawUnscaledSpotRectangle(Cairo::RefPtr<Cairo::Context> cr,
                                           int rectSize)
{
...
void CropWindow::getObservedFrameArea(int &x, int &y, int &w, int &h, int rw,
                                      int rh)
{

    int observedCropX, observedCropY, observedCropW, observedCropH;
    observedCropWin->getCropRectangle(observedCropX, observedCropY,
                                      observedCropW, observedCropH);
                                   ...
                                   int rh)
{
    ...
    cr->set_source_rgba(options.navGuideBrush[0], options.navGuideBrush[1],
                        ...
                        options.navGuideBrush[3]); //( 1, 1, 1, 1.0);
...
void CropWindow::cropImageUpdated()
{
    ...
    for (auto colorPicker : colorPickers) {
        ...
        cropHandler.colorPick(cropPos, r, g, b, rpreview, gpreview, bpreview,
                              colorPicker->getSize());
...
ImageArea *CropWindow::getImageArea() { return iarea; }

void CropWindow::setCropGUIListener(CropGUIListener *cgl) { cropgl = cgl; }

void CropWindow::setPointerMotionListener(PointerMotionListener *pml)
{
    pmlistener = pml;
    if (pml) {
        pml->signal_cycle_rgb().connect(
            sigc::mem_fun(*this, &CropWindow::cycleRGB));
        pml->signal_cycle_lch().connect(
            sigc::mem_fun(*this, &CropWindow::cycleLCH));
    }
}
