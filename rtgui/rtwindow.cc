...
MessageWindow::MessageWindow()
    : main_overlay_(nullptr), msg_revealer_(nullptr), info_label_(nullptr),
      info_msg_num_(0)
{
}
...
void MessageWindow::init(Gtk::Widget *main_widget)
{
    ...
    msg_revealer_ = Gtk::manage(new Gtk::Revealer());
    {
        ...
        info_image_ = icon;
        setExpandAlignProperties(icon, false, false, Gtk::ALIGN_CENTER,
                                 Gtk::ALIGN_START);
        ...
        msg_revealer_->set_transition_type(
...
{
    msg_revealer_->set_reveal_child(false);
    const auto hidebox = [this]() -> bool {
        info_box_->hide();
        return false;
    };
    Glib::signal_timeout().connect(sigc::slot<bool>(hidebox),
                                   msg_revealer_->get_transition_duration());
    info_msg_.clear();
...
void MessageWindow::show_info_msg(const Glib::ustring &descr, bool is_error,
                                  double duration, size_t padding)
{
    GThreadLock lock;

    const auto wrap = [padding](Glib::ustring s) -> Glib::ustring {
        Glib::ustring ret;
    ...
    if (reveal_conn_.connected()) {
        ...
        if (is_error) {
            ...
            if (n > 0) {
                msg =
                    Glib::ustring::compose(M("ERROR_MSG_MAXERRORS"), n) + "\n";
            }
        }
    }
    ...
    info_label_->set_markup(
        Glib::ustring::compose("<span size=\"large\"><b>%1</b></span>", msg));
    info_box_->show();
    info_image_->set_visible(is_error);
    msg_revealer_->set_reveal_child(true);
    reveal_conn_ = Glib::signal_timeout().connect(
        sigc::mem_fun(*this, &RTWindow::hide_info_msg),
        duration > 0
            ? duration
            : options.error_message_duration * (is_error ? 1.0 : 0.25));
}
...
RTWindow::RTWindow()
    : MessageWindow(), epanel(nullptr), fpanel(nullptr),
      ...
      is_application_(false)
{
    ...
    if (screen) {
        Gtk::Settings::get_for_screen(screen)->property_gtk_theme_name() =
            "Adwaita";
        ...
        if (GTK_MINOR_VERSION < 20 || options.theme != Options::DEFAULT_THEME) {
            Glib::RefPtr<Glib::Regex> regex =
                Glib::Regex::create(Options::THEMEREGEXSTR,
            ...
            if (match) {
                ...
                bool reset = false;
                if (!Glib::file_test(
                        Glib::build_filename(options.ART_base_dir, "themes",
                                             options.theme + ".css"),
                        Glib::FILE_TEST_EXISTS)) {
                    if (!deprecated) {
                        options.theme += "-DEPRECATED";
                        if (!Glib::file_test(Glib::build_filename(
                                                 options.ART_base_dir, "themes",
                                                 options.theme + ".css"),
                                             Glib::FILE_TEST_EXISTS)) {
                            reset = true;
                        }
                        ...
                        Glib::signal_timeout().connect(
                            sigc::slot<bool>([this, themeRootName]() -> bool {
                                error(Glib::ustring::compose(
                                    "%1: %2 - %3: %4", M("GENERAL_WARNING"),
                                    M("PREFERENCES_APPEARANCE_THEME"),
                                    themeRootName,
                                    M("GENERAL_DEPRECATED_TOOLTIP")));
                                return false;
                            }),
            ...
            filename = Glib::build_filename(options.ART_base_dir, "themes",
                                            options.theme + ".css");
    ...
    get_screen()->get_monitor_geometry(
        std::min(options.windowMonitor,
                 Gdk::Screen::get_default()->get_n_monitors() - 1),
        lMonitorRect);
    ...
    if (options.windowMaximized) {
        ...
        if (options.windowX <=
                lMonitorRect.get_x() + lMonitorRect.get_width() &&
            options.windowY <=
                lMonitorRect.get_y() + lMonitorRect.get_height()) {
    ...
    signal_key_press_event().connect(
        sigc::mem_fun(*this, &RTWindow::keyPressed));
    signal_key_press_event().connect(
        sigc::mem_fun(*this, &RTWindow::keyPressedBefore), false);
    signal_key_release_event().connect(
        sigc::mem_fun(*this, &RTWindow::keyReleased));
    signal_scroll_event().connect(
        sigc::mem_fun(*this, &RTWindow::scrollPressed), false);
    ...
    if (simpleEditor) {
        ...
        setExpandAlignProperties(fpanelLabelGrid, false, false,
                                 Gtk::ALIGN_CENTER, Gtk::ALIGN_CENTER);
        Gtk::Label *fpl = Gtk::manage(
            new Gtk::Label(Glib::ustring(" ") + M("MAIN_FRAME_EDITOR")));
        ...
        fpanelLabelGrid->set_tooltip_markup(
            M("MAIN_FRAME_FILEBROWSER_TOOLTIP"));
        ...
        // decorate tab, the label is unimportant since its updated in
        // batchqueuepanel anyway
        ...
        setExpandAlignProperties(preferences, false, false, Gtk::ALIGN_CENTER,
                                 Gtk::ALIGN_CENTER);
        ...
        setExpandAlignProperties(btn_fullscreen, false, false,
                                 Gtk::ALIGN_CENTER, Gtk::ALIGN_CENTER);
        ...
        setExpandAlignProperties(&prProgBar, false, false, Gtk::ALIGN_CENTER,
                                 Gtk::ALIGN_CENTER);
        ...
        setExpandAlignProperties(actionGrid, false, false, Gtk::ALIGN_CENTER,
                                 Gtk::ALIGN_CENTER);
...
void RTWindow::showErrors()
{
    ...
    if (options.is_defProfRawMissing()) {
        ...
        Gtk::MessageDialog msgd(
            *this,
            Glib::ustring::compose(M("OPTIONS_DEFRAW_MISSING"),
                                   options.defProfRaw),
    ...
    if (options.is_bundledDefProfRawMissing()) {
        Gtk::MessageDialog msgd(
            *this,
            Glib::ustring::compose(M("OPTIONS_BUNDLED_MISSING"),
                                   options.defProfRaw),
            true, Gtk::MESSAGE_ERROR, Gtk::BUTTONS_OK, true);
        msgd.run();
    ...
    if (options.is_defProfImgMissing()) {
        ...
        Gtk::MessageDialog msgd(
            *this,
            Glib::ustring::compose(M("OPTIONS_DEFIMG_MISSING"),
                                   options.defProfImg),
            true, Gtk::MESSAGE_ERROR, Gtk::BUTTONS_OK, true);
        msgd.run();
    }
...
void RTWindow::addEditorPanel(EditorPanel *ep, const std::string &name)
{
    if (options.multiDisplayMode > 0) {
        ...
        if (!EditWindow::isMultiDisplayEnabled()) {
            titleGrid->attach_next_to(*Gtk::manage(new RTImage("aperture.svg")),
                                      Gtk::POS_RIGHT, 1, 1);
        }
        titleGrid->attach_next_to(
            *Gtk::manage(new Gtk::Label(Glib::path_get_basename(name))),
            Gtk::POS_RIGHT, 1, 1);
...
void RTWindow::remEditorPanel(EditorPanel *ep)
{
    ...
    if (options.multiDisplayMode > 0) {
        ...
        // TODO: ask what to do: close & apply, close & apply selection, close &
        // revert, cancel
    }
}
...
bool RTWindow::selectEditorPanel(const std::string &name)
{
    if (options.multiDisplayMode > 0) {
        ...
        std::map<Glib::ustring, EditorPanel *>::iterator iep =
            epanels.find(name);
...
bool RTWindow::keyPressed(GdkEventKey *event)
{
    ...
    if (simpleEditor)
    // in simpleEditor mode, there's no other tab that can handle pressed keys,
    // so we can send the event to editor panel then return
    {
...
bool RTWindow::on_delete_event(GdkEventAny *event)
{
    ...
    // Check if any editor is still processing, and do NOT quit if so. Otherwise
    // crashes and inconsistent caches
    ...
    if (isSingleTabMode() || simpleEditor) {
        ...
        for (int i = 0; i < pageCount && !isProcessing; i++) {
            if (isEditorPanel(i)) {
                isProcessing |=
                    (static_cast<EditorPanel *>(mainNB->get_nth_page(i)))
                        ->getIsProcessing();
            }
        }
    }
    ...
    if ((isSingleTabMode() || simpleEditor) && epanel->isRealized()) {
        ...
        if (options.multiDisplayMode > 0 && editWindow) {
            ...
            // everything Look at the active panel first, if any, otherwise look
            // at the first one (sorted on the filename)

            int page = mainNB->get_current_page();
            Gtk::Widget *w = mainNB->get_nth_page(page);
            bool optionsWritten = false;

            for (std::map<Glib::ustring, EditorPanel *>::iterator i =
                     epanels.begin();
                 i != epanels.end(); ++i) {
                if (i->second == w) {
            ...
            if (!optionsWritten) {
                ...
                std::map<Glib::ustring, EditorPanel *>::iterator i =
                    epanels.begin();
                i->second->writeOptions();
            }
        }
...
void RTWindow::writeToolExpandedStatus(std::vector<int> &tpOpen)
{
    if ((isSingleTabMode() || gimpPlugin) && epanel->isRealized()) {
        ...
        // Storing the options of the last EditorPanel before Gtk destroys
        // everything Look at the active panel first, if any, otherwise look at
        // the first one (sorted on the filename)
        if (epanels.size()) {
            int page = mainNB->get_current_page();
            Gtk::Widget *w = mainNB->get_nth_page(page);
            bool optionsWritten = false;
...
void RTWindow::error(const Glib::ustring &descr) { showError(descr); }
...
void RTWindow::createSetmEditor()
{
    ...
    setExpandAlignProperties(editorLabelGrid, false, false, Gtk::ALIGN_CENTER,
                             Gtk::ALIGN_CENTER);
    Gtk::Label *const el = Gtk::manage(
        new Gtk::Label(Glib::ustring(" ") + M("MAIN_FRAME_EDITOR")));
    ...
    editorLabelGrid->attach_next_to(*Gtk::manage(new RTImage("aperture.svg")),
                                    pos, 1, 1);
